...
PATCH_FILE="$REPO_ROOT/patches/codex-full-tool-output.patch"
...
usage() {
  cat <<USAGE
Usage: ./scripts/install-source-stable.sh [options]
...
Examples:
  ./scripts/install-source-stable.sh                          # replace existing codex executable (default)
  ./scripts/install-source-stable.sh --no-in-place            # install to ~/.local/bin/codex
  ./scripts/install-source-stable.sh --install-dir ~/.local/bin
  ./scripts/install-source-stable.sh --codex-ref rust-v0.117.0
USAGE
}
